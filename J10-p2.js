...
function searchPath(position, reachsSet, trailHeadUid) {
    ...
    for (const direction of directions) {
        ...
        if (nextValue === 9 && currentValue === 8) {
            reachsSet.add(trailHeadUid + "/" + getCoordUid(nextPosition))
            continue;
        }
        ...
        const newTrailHeadUid = trailHeadUid + "/" + getCoordUid(nextPosition)
        searchPath({ value: nextValue, coord: nextPosition }, reachsSet, newTrailHeadUid);
    }
}
...
for (const start of starts) {
    ...
    searchPath({ coord: start, value: 0 }, reachsSet, getCoordUid(start));
